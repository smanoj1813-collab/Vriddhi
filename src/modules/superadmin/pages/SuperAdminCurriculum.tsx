...
// FIXED: Assigned tab, delete, Firestore colleges, assignment flow
// ═══════════════════════════════════════════════════════════════════════
...
import type { SyllabusExtract, CollegeOption } from '../types/curriculum';
...
export default function SuperAdminCurriculum() {
  ...
  const { extracts, items, loading: extractsLoading, total, approveExtract, refresh: refreshExtracts } = useSyllabusExtracts({ status: 'review' });
  const { stats, loading: statsLoading, refresh: refreshStats } = useCurriculumStats();
  ...
  const displayItems = extracts || items || [];
    ...
    <Box sx={{ p: 3, maxWidth: 1400, mx: 'auto' }}>
      ...
      <Paper sx={{ borderRadius: 2 }}>
        <Tabs value={tab} onChange={(_, v) => setTab(v)} sx={{ borderBottom: 1, borderColor: 'divider' }}>
          ...
          <Tab icon={<ReviewIcon />} iconPosition="start" label={`Review (${total})`} />
          <Tab icon={<AssignIcon />} iconPosition="start" label={`Assigned (${assignedCurriculum.length})`} />
        ...
        <Box sx={{ p: 3 }}>
          ...
          <TabPanel value={tab} index={0}>
            <SyllabusUploaderAny
              ...
              onExtractReady={handleExtractReady}
            />
          </TabPanel>
              ...
              <Box>
                <Box sx={{ display: 'flex', alignItems: 'center', gap: 2, mb: 2 }}>
                  ...
                  <Typography variant="h6">
                    Reviewing: {displayItems.find((e: SyllabusExtract) => e.id === reviewExtractId)?.fileName}
                  </Typography>
                </Box>
                    ...
                    <Box sx={{ display: 'flex', alignItems: 'center', gap: 2 }}>
                      <Box sx={{ flex: 1 }}>
                        ...
                        <Stack direction="row" spacing={1} sx={{ mt: 0.5 }}>
                          <Chip label={e.format.toUpperCase()} size="small" />
                          <Chip label={`${e.totalCourses} courses`} size="small" variant="outlined" />
                          <Chip label={`${e.totalModules} modules`} size="small" variant="outlined" />
                          <Chip label={`${e.totalHours} hrs`} size="small" variant="outlined" />
                          <Chip label={`${e.totalMarks} marks`} size="small" variant="outlined" />
                          <Chip
                            label={`${e.confidenceScore}% confidence`}
                            size="small"
                            color={e.confidenceScore >= 70 ? 'success' : e.confidenceScore >= 40 ? 'warning' : 'error'}
                          />
                        </Stack>
                      </Box>
                      <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                        ...
                        <IconButton
                          size="small"
                          color="error"
                          onClick={(ev) => handleDeleteExtract(e.id, ev)}
