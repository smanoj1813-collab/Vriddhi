...
const PaperGeneratorAdmin: React.FC = () => {
  const { user } = useAuth();
  ...
  const handleFilterChange = (field: keyof QuestionFilters, value: any) => {
    setFilters((prev: any) => ({ ...prev, [field]: value || undefined }));
    setLastDoc(null);
  };
      ...
      <Dialog open={formOpen} onClose={() => setFormOpen(false)} maxWidth="lg" fullWidth scroll="paper">
        ...
        <DialogContent dividers>
          <FacultyQuestionForm
            ...
            subjects={subjects}
            onSubmit={editingQuestion ? (data: any) => handleUpdate(editingQuestion.id, data) : handleCreate}
        ...
        <DialogContent dividers>
          {previewQuestion && (
            <Box>
              ...
              <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 2 }}>
                <Grid size={{ xs: 6 }}><Typography variant="body2" color="text.secondary">Subject: {previewQuestion.subject}</Typography></Grid>
                <Grid size={{ xs: 6 }}><Typography variant="body2" color="text.secondary">Type: {previewQuestion.type}</Typography></Grid>
                <Grid size={{ xs: 6 }}><Typography variant="body2" color="text.secondary">Difficulty: {previewQuestion.difficulty}</Typography></Grid>
                <Grid size={{ xs: 6 }}><Typography variant="body2" color="text.secondary">Unit: {previewQuestion.unit}</Typography></Grid>
                <Grid size={{ xs: 6 }}><Typography variant="body2" color="text.secondary">Batch: {previewQuestion.batch || 'N/A'}</Typography></Grid>
                <Grid size={{ xs: 6 }}><Typography variant="body2" color="text.secondary">Branch: {previewQuestion.branch || 'N/A'}</Typography></Grid>
                {previewQuestion.isPYQ && (
                  <Grid size={{ xs: 12 }}><Chip icon={<SchoolIcon />} label={`PYQ: ${previewQuestion.examYear} - ${previewQuestion.examName}`} color="secondary" size="small" /></Grid>
                )}
              </Box>
...
export default PaperGeneratorAdmin;
