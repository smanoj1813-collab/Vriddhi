    ...
    <div className="min-h-screen bg-slate-950">
      ...
      <header className="sticky top-0 z-30 bg-slate-950/80 backdrop-blur-xl border-b border-slate-800/50">
        <div className="px-6 py-4 flex items-center justify-between">
          <div>
            <h1 className="text-2xl font-bold text-white">Onboarding Center</h1>
            <p className="text-sm text-slate-400">Bulk upload students, faculty, schedules & assessments</p>
          </div>
          <div className="flex items-center gap-3">
            <span className="text-sm text-slate-400">College Admin</span>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            >
              <div className="flex items-start justify-between">
                <div>
                  <h3 className="text-lg font-semibold text-white">{currentTab.template.name}</h3>
                  <p className="text-sm text-slate-400 mt-1">{currentTab.template.description}</p>
                </div>
                <button
                  onClick={handleDownloadTemplate}
                  className="flex items-center gap-2 px-4 py-2 rounded-lg bg-slate-800 hover:bg-slate-700 text-teal-400 text-sm font-medium transition-colors"
                >
                  <Download size={14} />
                  Download Template
                </button>
              ...
              <div className="mt-4">
                ...
                <div className="flex flex-wrap gap-2">
                  {currentTab.template.fields.filter((f: TemplateField) => f.required).map((field: TemplateField) => (
                    <span key={field.key} className="px-2.5 py-1 text-xs font-medium bg-red-500/10 text-red-400 rounded-md border border-red-500/20">
                      {field.name}
                    </span>
                  ))}
                </div>
              ...
              <div className="mt-3">
                ...
                <div className="flex flex-wrap gap-2">
                  {currentTab.template.fields.filter((f: TemplateField) => !f.required).map((field: TemplateField) => (
                    <span key={field.key} className="px-2.5 py-1 text-xs font-medium bg-slate-700/50 text-slate-400 rounded-md">
                      {field.name}
                    </span>
                  ))}
                </div>
            ...
            >
              ...
              {uploadedFile ? (
                <>
                  <p className="text-sm font-medium text-teal-400">{uploadedFile.name}</p>
                  <p className="text-xs text-slate-500 mt-1">{(uploadedFile.size / 1024).toFixed(1)} KB</p>
                </>
              ) : (
                <>
                  <p className="text-sm text-slate-300 font-medium">Drop CSV/Excel file here or click to browse</p>
                ...
                >
                  <div className="p-4 border-b border-slate-700/30 flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-4 text-sm">
                      <span className="text-slate-400">
                        <span className="text-white font-medium">{parseResult.data.length}</span> valid
                      </span>
                      <span className="text-slate-400">
                    ...
                    <div className="max-h-48 overflow-y-auto">
                      {parseResult.errors.slice(0, 20).map((error: OnboardingError, i: number) => (
                        <div key={i} className="px-4 py-2.5 border-b border-slate-700/30 flex items-start gap-3">
                          ...
                          <div>
                            <p className="text-xs text-slate-400">Row {error.rowNumber} • {error.regNoOrId}</p>
                            <p className="text-sm text-red-400">{error.field}: {error.message}</p>
                          </div>
                        </div>
                      ))}
                      {parseResult.errors.length > 20 && (
                        <p className="px-4 py-2 text-xs text-slate-500">...and {parseResult.errors.length - 20} more errors</p>
                      )}
                    </div>
                  )}
                  ...
                  {parseResult.data.length > 0 && showPreview && (
                    <div className="overflow-x-auto">
                      <table className="w-full text-sm">
                        ...
                        <tbody className="divide-y divide-slate-700/30">
                          {parseResult.data.slice(0, 5).map((row: Record<string, string>, i: number) => (
                            <tr key={i}>
                              {currentTab.template.fields.slice(0, 6).map((f: TemplateField) => (
                                <td key={f.key} className="px-4 py-2 text-slate-300 truncate max-w-[150px]">{row[f.key] || '-'}</td>
                              ))}
                            </tr>
                          ))}
                        </tbody>
                      ...
                      {parseResult.data.length > 5 && (
                        <p className="px-4 py-2 text-xs text-slate-500">...and {parseResult.data.length - 5} more rows</p>
                      )}
                    </div>
                  )}
                  ...
                  <div className="p-4 border-t border-slate-700/30 flex justify-end gap-3">
                    ...
                    <button
                      ...
                      className="px-6 py-2 rounded-lg text-sm font-medium bg-teal-500 hover:bg-teal-400 disabled:bg-slate-700 disabled:text-slate-500 text-white transition-colors"
                    >
          ...
          <div className="space-y-6">
            <div className="glass-card rounded-xl border border-slate-700/30 p-5">
              <h3 className="text-lg font-semibold text-white mb-4">How to Onboard</h3>
            ...
            <div className="glass-card rounded-xl border border-slate-700/30 p-5">
              <h3 className="text-lg font-semibold text-white mb-4">Field Guidelines</h3>
                ...
                {currentTab.template.fields.map((field: TemplateField) => (
                  <div key={field.key} className="p-3 rounded-lg bg-slate-800/30">
                    <div className="flex items-center gap-2">
                      ...
                      {field.required && (
                        <span className="px-1.5 py-0.5 text-[10px] font-bold bg-red-500/10 text-red-400 rounded">REQUIRED</span>
                      )}
                    </div>
