...
// Creating identities — students, faculty, HOD, principal — is superadmin-only.
// That is enforced server-side by verifyCaller(request, ['superadmin']) in
// bulkCreateStudentAccounts and bulkProvisionStaff, so enabling import here
// would only produce permission errors. A college admin gets validation only:
// check the file here, then send it to a superadmin to provision.
const TABS = [
  { id: 'students', label: 'Students', icon: Users, template: STUDENT_TEMPLATE, importable: false },
  { id: 'faculty', label: 'Faculty', icon: GraduationCap, template: FACULTY_TEMPLATE, importable: false },
                  ...
                  <div className="p-4 border-t border-slate-700/30 flex justify-end gap-3">
                    ...
                    <div className="flex flex-col items-end gap-1.5">
                      {!currentTab.importable && (
                        <span className="text-xs text-amber-500 max-w-xs text-right">
                          Validation only — creating {currentTab.label.toLowerCase()} accounts is
                          restricted to a super admin. Check the file here, then send it to one to import.
                        </span>
                      )}
                    </div>
                  </div>
