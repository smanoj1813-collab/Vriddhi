  ...
  return (
    <div className="mx-auto max-w-5xl space-y-6 pb-10">
      ...
      <section className="overflow-hidden rounded-3xl bg-gradient-to-br from-teal-600 via-teal-700 to-indigo-800 p-6 text-white shadow-lg sm:p-8">
        ...
        <div className="mt-5 rounded-2xl bg-slate-500/10 dark:bg-white/10 p-4 backdrop-blur">
          ...
          return (
            <Card key={mod.id} className="p-0">
              ...
              >
                ...
                {expanded ? <ChevronDown className="mt-1 h-5 w-5 shrink-0 text-slate-500 dark:text-slate-400" /> : <ChevronRight className="mt-1 h-5 w-5 shrink-0 text-slate-500 dark:text-slate-400" />}
              </button>
                      ...
                      const row = (
                        <div className={`-mx-2 flex items-start gap-3 rounded-lg px-2 py-2.5 ${topicUnlocked ? 'hover:bg-slate-50/80 dark:hover:bg-slate-800/40' : 'opacity-60'}`}>
                          {!topicUnlocked ? (
                            ...
                            <Circle className="mt-0.5 h-5 w-5 shrink-0 text-slate-300 dark:text-slate-400" />
                          )}
                          <span className="min-w-0 flex-1">
                            <span className="flex flex-wrap items-center gap-x-2">
                              <span className="text-xs font-bold text-slate-500 dark:text-slate-400">{topic.number}</span>
                            ...
                            {!topicUnlocked ? <span className="mt-0.5 block text-[10px] text-slate-500 dark:text-slate-400">Read the previous topic and submit its quiz to unlock.</span> : null}
                          </span>
          ...
          {manifest.assessment.conditions?.length ? (
            <ul className="mt-3 space-y-1">
              {manifest.assessment.conditions.map((c, idx) => (
                <li key={idx} className="flex items-start gap-2 text-xs text-slate-600 dark:text-slate-400">
                  <ClipboardCheck className="mt-0.5 h-3.5 w-3.5 shrink-0 text-slate-500 dark:text-slate-400" /> {c}
                </li>
              ))}
            </ul>
          ) : null}
            ...
            {(manifest.toolkit || []).map((group) => (
              <li key={group.category} className="text-sm">
                <span className="flex items-center gap-1.5 font-semibold text-slate-800 dark:text-slate-100">
                  <Wrench className="h-3.5 w-3.5 text-slate-500 dark:text-slate-400" /> {group.category}
                </span>
      ...
      <Card>
        ...
        <ul className="mt-4 grid grid-cols-1 gap-2 sm:grid-cols-2">
          {certificate.checks.map((check) => (
            <li key={check.id} className={`flex items-start gap-2 rounded-xl border px-3 py-2 ${check.passed ? 'border-emerald-200 bg-emerald-50/70 dark:border-emerald-900 dark:bg-emerald-950/20' : 'border-slate-200 bg-slate-50 dark:border-slate-800 dark:bg-slate-800/40'}`}>
              {check.passed ? <CheckCircle2 className="mt-0.5 h-4 w-4 shrink-0 text-emerald-600" /> : <Circle className="mt-0.5 h-4 w-4 shrink-0 text-slate-500 dark:text-slate-400" />}
        ...
        <p className="mt-3 text-[10px] leading-relaxed text-slate-500 dark:text-slate-400">This download verifies course-platform completion (all content, quizzes and module assessments). College credit, attendance, project grading and academic-integrity clearance may be governed by additional college rules.</p>
      </Card>
...
function Stat({ icon, label, value }: { icon: React.ReactNode; label: string; value: string }) {
  return (
    <div className="rounded-xl bg-slate-500/10 dark:bg-white/10 px-3 py-2">
