...
const statusConfig: Record<AssignmentStatus, { color: string; label: string; icon: React.ReactNode }> = {
  draft: { color: 'bg-slate-500/10 text-slate-400 border-slate-500/20', label: 'Draft', icon: <FileText className="w-4 h-4" /> },
  published: { color: 'bg-blue-500/10 text-blue-400 border-blue-500/20', label: 'Published', icon: <Send className="w-4 h-4" /> },
  ongoing: { color: 'bg-amber-500/10 text-amber-400 border-amber-500/20', label: 'Ongoing', icon: <Clock className="w-4 h-4" /> },
    ...
    <div className="p-6 lg:p-8 max-w-7xl mx-auto min-h-screen">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-slate-400 text-sm">Create, track, and grade student assignments</p>
          </div>
        </div>
        <button
      ...
      <div className="flex flex-col sm:flex-row gap-3 mb-6">
        ...
        <select
          ...
          className="bg-slate-800/50 border border-slate-700/50 rounded-xl px-3 py-2.5 text-white text-sm focus:outline-none focus:border-teal-500/50"
        >
                ...
                <div className="flex flex-col lg:flex-row lg:items-start gap-4">
                  <div className="flex-1">
                    ...
                    <div className="flex flex-wrap gap-4 text-xs text-slate-400">
                      <span className="flex items-center gap-1"><Target className="w-3.5 h-3.5 text-teal-400" /> {item.topic}</span>
                      <span className="flex items-center gap-1"><Calendar className="w-3.5 h-3.5 text-amber-400" /> Due: {item.deadline}</span>
                      <span className="flex items-center gap-1"><Users className="w-3.5 h-3.5 text-blue-400" /> {item.batch}</span>
                      <span className="flex items-center gap-1"><BarChart3 className="w-3.5 h-3.5 text-purple-400" /> Max: {item.maxScore} pts</span>
                    </div>
                    ...
                    {item.submissions.length > 0 && (
                      <div className="mt-3 flex items-center gap-4">
                        <div className="flex items-center gap-2 text-xs">
                          ...
                          <span className="text-slate-300">{submissionRate}%</span>
                        </div>
                        <span className="text-xs text-slate-400">Avg Score: <span className="text-slate-200">{avgScore}</span></span>
                      </div>
                    )}
                  </div>
                  ...
                  <div className="flex gap-2 shrink-0">
                    <button
                      ...
                      className="px-3 py-1.5 rounded-xl bg-teal-500/10 text-teal-400 border border-teal-500/20 text-sm font-medium hover:bg-teal-500/20 transition-all"
                    >
                      View Submissions
                    </button>
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-slate-800 border border-slate-700 rounded-2xl w-full max-w-lg p-6 max-h-[90vh] overflow-y-auto">
            <div className="flex items-center justify-between mb-6">
              ...
              <button onClick={() => { setShowCreate(false); resetCreateForm() }} className="p-1.5 rounded-lg hover:bg-slate-700 transition-colors">
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <div>
                  <label className="block text-sm text-slate-400 mb-1.5">Max Score</label>
                  <input
                    type="number"
      ...
      {showDetail && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-slate-800 border border-slate-700 rounded-2xl w-full max-w-3xl p-6 max-h-[90vh] overflow-y-auto">
            <div className="flex items-center justify-between mb-6">
              <div>
                <h2 className="text-xl font-bold text-white">{showDetail.title}</h2>
                <p className="text-sm text-slate-400 mt-1">{showDetail.topic} • Due: {showDetail.deadline} • Max: {showDetail.maxScore} pts</p>
              </div>
              <button onClick={() => setShowDetail(null)} className="p-1.5 rounded-lg hover:bg-slate-700 transition-colors">
                <X className="w-5 h-5 text-slate-400" />
              </button>
            </div>

            <p className="text-slate-300 text-sm mb-6 bg-slate-700/30 rounded-xl p-4 border border-slate-700/50">{showDetail.description}</p>
            ...
            <div className="space-y-2">
              <h3 className="text-sm font-semibold text-slate-300 mb-3">Student Submissions</h3>
                ...
                showDetail.submissions.map(sub => {
                  ...
                  return (
                    <div key={sub.studentId} className="flex items-center gap-4 p-3 rounded-xl bg-slate-700/30 border border-slate-700/50">
                      <div className="w-8 h-8 rounded-full bg-slate-600 flex items-center justify-center text-sm font-semibold text-slate-300 shrink-0">
                        {sub.studentName.charAt(0)}
                      </div>
                      <div className="flex-1 min-w-0">
                        <p className="text-sm font-medium text-white">{sub.studentName}</p>
                        <p className="text-xs text-slate-400">{sub.rollNo}</p>
                      </div>
                      {sub.submittedAt && (
                      ...
                      {sub.score !== undefined && (
                        <span className="text-sm font-semibold text-white shrink-0">{sub.score}/{sub.maxScore}</span>
                      )}
                      ...
                      {sub.status === 'graded' && sub.remarks && (
                        <span className="text-xs text-slate-400 truncate max-w-[120px] shrink-0" title={sub.remarks}>{sub.remarks}</span>
                      )}
                    </div>
                  )
                })
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-[60] p-4">
          <div className="bg-slate-800 border border-slate-700 rounded-2xl w-full max-w-md p-6">
            <div className="flex items-center justify-between mb-6">
              ...
              <button onClick={() => setShowGradeModal(null)} className="p-1.5 rounded-lg hover:bg-slate-700 transition-colors">
                <X className="w-5 h-5 text-slate-400" />
              </button>
            </div>

            <div className="mb-4 p-3 bg-slate-700/30 rounded-xl border border-slate-700/50">
              <p className="text-sm text-slate-300"><span className="text-slate-500">Student:</span> {showGradeModal.studentName}</p>
              <p className="text-sm text-slate-300"><span className="text-slate-500">Roll No:</span> {showGradeModal.rollNo}</p>
              <p className="text-sm text-slate-300"><span className="text-slate-500">Max Score:</span> {showGradeModal.maxScore}</p>
            </div>

            <div className="space-y-4">
              <div>
                <label className="block text-sm text-slate-400 mb-1.5">Score</label>
                <input
                  type="number"
                  value={gradeScore}
