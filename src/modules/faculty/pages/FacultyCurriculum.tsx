  ...
  if (loading && curriculum.length === 0) {
    return (
      <div className="p-6 lg:p-8 max-w-7xl mx-auto flex flex-col items-center justify-center min-h-[60vh]">
        ...
        <p className="text-slate-500 dark:text-slate-400">Loading your curriculum...</p>
      </div>
    )
  }
  ...
  return (
    <div className="p-6 lg:p-8 max-w-7xl mx-auto">
      ...
      <div className="flex items-center gap-4 mb-6">
        <Link
          to="/faculty"
          className="p-2 rounded-lg bg-slate-100 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 hover:border-teal-500/30 text-slate-500 dark:text-slate-400 hover:text-teal-500 transition-all"
        >
          <ArrowLeft className="w-5 h-5" />
        </Link>
        <div className="flex-1">
          <h1 className="text-2xl font-bold text-slate-900 dark:text-white">My Curriculum</h1>
      ...
      {stats && (
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-6">
          ...
          <div className="p-4 rounded-xl bg-amber-500/5 border border-amber-500/20">
            ...
            <p className="text-2xl font-bold text-amber-400">{stats.totalHours}h</p>
          </div>
              ...
              >
                <div className="flex items-start justify-between mb-2">
                  <div>
                    ...
                    <p className="text-xs text-slate-500 dark:text-slate-400">{cls.subjectCode}</p>
                  </div>
        ...
        <div>
          {curriculum.length === 0 ? (
            <div className="p-12 text-center rounded-xl bg-slate-100 dark:bg-slate-800/30 border border-slate-200 dark:border-slate-700 border-dashed">
              ...
              <p className="text-slate-500 dark:text-slate-400">No curriculum assigned yet</p>
              ...
              {filteredCurriculum.map((course: FacultyCurriculumView) => {
                ...
                return (
                  ...
                  >
                    {/* Course Header */}
                    <div
                      className="p-4 flex items-center gap-4 cursor-pointer hover:bg-slate-200/50 dark:hover:bg-slate-700/30 transition-colors"
                      onClick={() => {
                    ...
                    >
                      ...
                      <div className="flex items-center gap-4 text-sm">
                        ...
                        <div className="text-center hidden sm:block">
                          ...
                          <p className="text-slate-900 dark:text-white font-medium">{course.credits}</p>
                        </div>
                    ...
                    {isExpanded && (
                      <div className="px-4 pb-4 border-t border-slate-200 dark:border-slate-700 pt-4">
                        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                          {/* Modules Preview */}
                          <div>
                            <h4 className="text-xs text-slate-400 mb-3 flex items-center gap-1">
                            ...
                            <div className="space-y-2 max-h-64 overflow-y-auto">
                              {course.modules.map((mod: ParsedModule) => (
                                <div
                                  key={mod.id}
                                  className="p-3 rounded-lg bg-slate-200/50 dark:bg-slate-900/50 border border-slate-200 dark:border-slate-700/50"
                                >
                                  ...
                                  <p className="text-sm text-slate-300">{(mod.title || mod.moduleName || '')}</p>
                                  {mod.topics && mod.topics.length > 0 && (
                                    <div className="flex flex-wrap gap-1 mt-2">
                                      ...
                                      {mod.topics.length > 3 && (
                                        <span className="text-xs text-slate-500">+{mod.topics.length - 3} more</span>
                                      )}
                                    </div>
                                  )}
                                </div>
                              ...
                              <div className="space-y-2 max-h-64 overflow-y-auto">
                                {courseSchedules
                                  ...
                                  .map((cls: FacultyScheduleItem) => (
                                    <div
                                      key={cls.id}
                                      className="p-3 rounded-lg bg-slate-200/50 dark:bg-slate-900/50 border border-slate-200 dark:border-slate-700/50 flex items-center justify-between"
                                    >
                                      <div>
                                        <div className="flex items-center gap-2">
                                        ...
                                        <div className="flex items-center gap-2 mt-1">
                                          ...
                                          <span className="text-xs text-slate-500">Room {cls.room}</span>
                                        </div>
                                      </div>
                        ...
                        <div className="flex gap-2 mt-4 pt-4 border-t border-slate-200 dark:border-slate-700">
                          ...
                          <button
                            ...
                            className="flex-1 px-3 py-2 rounded-lg text-xs bg-blue-500/10 text-blue-400 border border-blue-500/20 hover:bg-blue-500/20 transition-all flex items-center justify-center gap-1"
                          >
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {courseModules.map((mod: ParsedModule) => (
              ...
              >
                ...
                {mod.topics && mod.topics.length > 0 && (
                  <div className="mb-3">
                    <p className="text-xs text-slate-500 dark:text-slate-400 mb-2">Topics:</p>
                ...
                {mod.learningOutcomes && mod.learningOutcomes.length > 0 && (
                  <div>
                    <p className="text-xs text-slate-500 dark:text-slate-400 mb-2">Learning Outcomes:</p>
        ...
        <div>
          ...
          {courseSchedule.length === 0 ? (
            <div className="p-12 text-center rounded-xl bg-slate-100 dark:bg-slate-800/30 border border-slate-200 dark:border-slate-700 border-dashed">
              ...
              <p className="text-slate-500 dark:text-slate-400">No schedule entries yet</p>
              <p className="text-sm text-slate-400 dark:text-slate-500 mt-1">
                ...
                >
                  ...
                  {cls.topicsPlanned && cls.topicsPlanned.length > 0 && (
                    <div className="mt-3 pt-3 border-t border-slate-200 dark:border-slate-700">
                      <p className="text-xs text-slate-500 dark:text-slate-400 mb-2">Topics Planned:</p>
                      <div className="flex flex-wrap gap-1.5">
      ...
      {(activeView === 'modules' || activeView === 'schedule') && !selectedCourseData && (
        <div className="p-12 text-center rounded-xl bg-slate-100 dark:bg-slate-800/30 border border-slate-200 dark:border-slate-700 border-dashed">
          <BookOpen className="w-10 h-10 text-slate-400 mx-auto mb-3" />
          <p className="text-slate-500 dark:text-slate-400">Select a course from the Overview tab first</p>
          <button
            onClick={() => setActiveView('overview')}
