    ...
    <div className="p-6 lg:p-8 max-w-7xl mx-auto min-h-screen">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-2xl font-bold text-slate-900 dark:text-white">Upload Material</h1>
            <p className="text-slate-600 dark:text-slate-400 text-sm">Manage course materials and resources</p>
          </div>
        </div>
      ...
      {loading && materials.length === 0 && (
        <div className="flex items-center justify-center py-16">
          ...
          <span className="ml-3 text-slate-600 dark:text-slate-400">Loading materials...</span>
        </div>
      )}
      ...
      <div className="space-y-3">
        {materials.length === 0 && !loading ? (
          <div className="bg-white/60 dark:bg-slate-800/30 border border-slate-200 dark:border-slate-700/30 rounded-2xl p-12 text-center">
            <FolderOpen className="w-12 h-12 text-slate-400 dark:text-slate-600 mx-auto mb-3" />
          ...
          materials.map(material => (
            <div key={material.id} className="bg-white dark:bg-slate-800/50 border border-slate-200 dark:border-slate-700/50 rounded-2xl p-4 hover:border-slate-300 dark:hover:border-slate-600 transition-all group shadow-sm">
              <div className="flex items-center gap-4">
                <div className="p-3 rounded-xl bg-slate-100 dark:bg-slate-700/30 shrink-0">
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-2xl w-full max-w-lg p-6 max-h-[90vh] overflow-y-auto shadow-xl">
            <div className="flex items-center justify-between mb-6">
              ...
              <button onClick={resetModal} className="p-1.5 rounded-lg hover:bg-slate-100 dark:hover:bg-slate-700 transition-colors">
            ...
            <div className="space-y-4">
                ...
                <div
                  ...
                  className={`border-2 border-dashed rounded-xl p-8 text-center cursor-pointer transition-all ${
                    dragOver
                      ...
                      : 'border-slate-300 dark:border-slate-600 hover:border-slate-400 dark:hover:border-slate-500 bg-slate-50 dark:bg-slate-700/20'
                  }`}
                >
                  <input
                    ref={fileInputRef}
              ...
              {uploadType === 'link' && (
                <div>
                  <label className="block text-sm text-slate-600 dark:text-slate-400 mb-1.5">URL *</label>
                  <input
                    type="url"
                    value={uploadUrl}
